...
namespace JW.AUBE.Core.Forms.Inventory
{
	public partial class InvAdjsForm : EditForm
	{
		...
		void DetailDataLoad(object id)
		{
			try
			{
				var res = ServerRequest.GetData("InvAdjs", new DataMap() { { "ADJS_ID", id } });
				if (res.DataList.Count > 0)
				{
					if (res.DataList[0].Data == null)
						throw new Exception("조회 데이터가 없습니다.");

					InvAdjsDataModel model = (InvAdjsDataModel)res.DataList[0].Data;
		...
		protected override void DataSave(object arg, SaveCallback callback)
		{
			try
			{
				...
				var res = ServerRequest.Execute("InvAdjs", "Save", new DataTable[] { map.ToDataTable() });
		...
		protected override void DataDelete(object arg, DeleteCallback callback)
		{
			try
			{
				...
				var res = ServerRequest.SingleRequest("InvAdjs", "Save", null, dt);
				if (res.ErrorNumber != 0)
					throw new Exception(res.ErrorMessage);
